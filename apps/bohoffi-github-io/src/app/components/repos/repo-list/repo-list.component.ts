...
@Component({
  ...
  standalone: true,
  imports: [AsyncPipe, RepoNamePipe]
})
export class RepoListComponent implements OnInit {
  ...
  public ngOnInit(): void {
    const $bohoffi = this.repos
      .userRepositories('bohoffi')
      .pipe(map((repos: Repository[]) => repos.filter((repo: Repository) => !repo.fork && repo.stargazers_count > 0)));
    const $baltic = this.repos
      .userRepositories('balticcode')
      .pipe(map((repos: Repository[]) => repos.filter((repo: Repository) => !repo.fork && repo.stargazers_count > 0)));
    this.$repos = forkJoin({
      bohoffi: $bohoffi,
      balticCode: $baltic
